...
//	}
	...
	public void findAll_테스트() throws Exception {
		...
		resultAction.andExpect(status().isOk()).andExpect(jsonPath("$", Matchers.hasSize(3)))
				.andExpect(jsonPath("$.[2].title").value("JUnit따라하기")).andDo(MockMvcResultHandlers.print());
	...
	public void findById_테스트() throws Exception {
		...
		// then
		resultAction.andExpect(status().isOk()).andExpect(jsonPath("$.title").value("리액트따라하기"))
				.andDo(MockMvcResultHandlers.print());
	...
	public void update_테스트() throws Exception {
		...
		// when
		ResultActions resultAction = mockMvc.perform(get("/book/{id}", id).contentType(MediaType.APPLICATION_JSON_UTF8)
				.content(content).accept(MediaType.APPLICATION_JSON_UTF8));
